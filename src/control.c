...
void poll_control(Tcontrol *c) {
        ...
        if (c->use_joy) {
                ...
                if (joy[0].button[1].b) c->flags |= K_JUMP;
                if (joy[0].button[2].b) c->flags |= K_FIRE;
                if (joy[0].button[3].b) c->flags |= K_SPECIAL;
        }
...
int is_special(Tcontrol *c)  { return (c->flags & K_SPECIAL  ? TRUE : FALSE); }
