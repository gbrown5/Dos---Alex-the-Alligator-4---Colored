...
char scroller_message[] =
        ...
        "Use Arrows to move Alex, Alt to jump and LCtrl to shoot      "
        "Code and GFX by Johan Peitz      Music and SFX by Anders Svensson    MSDOS port by Gbrownie     "
...
PALETTE palette_white;          // all-white palette for fades
...
void log2file(char *format, ...) {
        if(do_log == 0) return; //bail if user doesn't want to log
...
void wait_key(int seconds) {
        ...
        while(!kp && t < seconds * 50) {
                ...
                t ++;
                if (keypressed()) kp = 1;
                if (is_fire(&ctrl) || is_jump(&ctrl)) kp = 1;   // by MK2k
                //if (is_fire(&ctrl)) kp = 1;
                if (got_sound && dp != NULL) al_poll_duh(dp);
                while(!cycle_count);
        }
}
...
// Argh! - handles fades in full color mode
void do_fade(PALETTE pal, int fadein, int delay, AL_DUH_PLAYER *duh_player) {
        int steps, i;
        PALETTE temp;

        if (delay < 40) // helps compensate for short delays
                steps = 3;
        else {
                steps = 6;
                delay /= 2;
...
int init_game(const char *map_file) {
    ...
    dumb_register_dat_mod(DUMB_DAT_MOD);
        ...
        */
        ...
        if (i != -770405) {
                if (install_sound(i, MIDI_AUTODETECT, NULL)) {
                        log2file("  *** failed");
                        got_sound = 0;
                }
                else got_sound = 1;
...
void show_custom_ending() {
        ...
        while(!done) {
                ...
                while(cycle_count > 0) {
                        ...
                        // poll user
                        poll_control(&ctrl);
                        if (is_fire(&ctrl) || is_jump(&ctrl) || key[KEY_SPACE] || key[KEY_ENTER] || key[KEY_ESC]) // by MK2k
                        //if (is_fire(&ctrl) || key[KEY_SPACE] || key[KEY_ENTER] || key[KEY_ESC]) // by MK2k
                                done = 1;
...
// Argh! - tint palette to specified level using specified palette,
//         (replaces transform_bitmap()-style tints in full color mode)
void tint_palette(PALETTE tint, int level) {
        PALETTE temp;

        fade_interpolate(tint, org_pal, temp, level, 0, 255);
        // main game text isn't tinted (but GUI is)
...
void show_cutscene(int level) {
        ...
        while(mode != 3) {
                ...
                while(cycle_count > 0) {
                        ...
                        if (mode == 1 && ((keypressed() || is_fire(&ctrl) || is_jump(&ctrl) ) || my_counter > 200)) { // by MK2k
                                mode = 2;
                        }
...
void show_scores(int space, Thisc *table) {
        ...
        draw_hisc_table(table, swap_screen, data[THE_FONT].dat, 10, 30, (space ? color_text_white : color_text_black), !space);
...
int select_starting_level() {
        ...
        while(!done) {
                ...
                while(cycle_count > 0) {
                        ...
                        //if (is_fire(&ctrl)) { // by MK2k
                                done = 1;
                                play_sound(sfx[SMPL_MENU]);
                        }

                        // by MK2k - start the special stage by pressing SQUARE
                        if (is_special(&ctrl) && options.one_hundred) {
                                        done = 1;
                                        start_level = -100;
                                        play_sound(sfx[SMPL_MENU]);
                        }
...
void update_player() {
        ...
        if (player.actor->status != AC_DEAD) {
                ...
                // DEBUG jumping analysis by mk2k
                /*
                if (is_jump(&ctrl) && player.jumping) {
                        log2file(" Jump but already jumping!");
                }
                if (is_jump(&ctrl) && player.jump_pressed) {
                        log2file(" Jump but already pressed!");
                }
                */
...
int do_pause_menu(BITMAP *bg) {
        ...
        // wait to release esc or joyb
        while(key[KEY_ESC]|| joy[0].button[0].b);
        ...
        while(!done) {
                ...
                if (key[KEY_ESC] || joy[0].button[0].b) done = -1;
                yield_timeslice();
        }
...
int play(int level) {
        ...
        while(game_status == GS_OK) {
                ...
                while(cycle_count > 0) {
                        ...
                        // COMMON STUFF
                        if (key[KEY_ESC] || joy[0].button[0].b) {
...
void draw_title(BITMAP *bmp, int tick) {
        ...
        //if (!playing_original_game) strcpy(start_string, "CUSTOM GAME"); // by MK2k
...
void draw_custom_menu(BITMAP *bmp, int tick) {
        ...
        y += step;
        textout(bmp, data[THE_FONT].dat, "CUSTOM 2", x+1, y+1, 1);
        textout(bmp, data[THE_FONT].dat, "CUSTOM 2", x, y, 4);

        y += step;
        textout(bmp, data[THE_FONT].dat, "CUSTOM 3", x+1, y+1, 1);
        textout(bmp, data[THE_FONT].dat, "CUSTOM 3", x, y, 4);
...
int do_main_menu() {
        ...
        while(status == GS_OK) {
                ...
                while(cycle_count > 0) {
                        ...
                        if (!count) {
                                ...
                                //if (key[KEY_SPACE] || key[KEY_ENTER] || is_fire(&ctrl)) {     // by MK2k
                                        if (menu_choice == 1) {
        ...
        if (status == GS_CUSTOM) {      // User selected Custom
                fix_gui_colors();
                log2file("\nEntering custom menu");
                fade_out_pal(100);

                do_custom_menu();
        }
        else if (status == GS_EDIT) {   // User selected EDIT
                stop_music();
                fix_gui_colors();
                log2file("\nEntering editor:");
                fade_out_pal(100);
                ...
                if (level == 0) // by MK2k, show intro on first level
                {
                        stop_music();
        ...
        if (status == GS_QUIT_MENU) {
                ...
                textout_outline_center(swap_screen, "Thanks for playing!", 80, 0);
                textout_outline_center(swap_screen, "Design, Code, GFX:", 80, 20);
                textout_outline_center(swap_screen, "Johan Peitz", 80, 32);
                textout_outline_center(swap_screen, "MUSIC, SFX:", 80, 46);
                textout_outline_center(swap_screen, "Anders Svensson", 80, 58);
                textout_outline_center(swap_screen, "MSDOS Port:", 80, 80);
                textout_outline_center(swap_screen, "Gbrownie", 80, 92);
                blit_to_screen(swap_screen);
                fade_in_pal(60);
                tint_palette(palette_black, 53);
                fade_rest(40, dp);
                ...
                fade_rest(40, dp);
                cycle_count = 0;
                while(!key[KEY_ESC] && cycle_count < 300);
                fade_out_pal(100);
                clear(screen);
                game_status = GS_QUIT_MENU; // Argh! - signal to exit game
        }
...
int do_custom_menu() {
        ...
        while(status == GS_OK) {
                ...
                while(cycle_count > 0) {
                        ...
                        if (!count) {
                                ...
                                //if (key[KEY_SPACE] || key[KEY_ENTER] || is_fire(&ctrl)) {     // by MK2k
                                        if (menu_choice == 1) {
                                                log2file(" custom1 selected");
                                                status = GS_PLAY;
                                                play_sound(sfx[SMPL_MENU]);
                                        }
                                        if (menu_choice == 2) {
                                                log2file(" custom2 selected");
                                                status = GS_PLAY;
                                                play_sound(sfx[SMPL_MENU]);
                                        }
                                        if (menu_choice == 3) {
                                                log2file(" custom3 selected");
                                                status = GS_PLAY;
                                                play_sound(sfx[SMPL_MENU]);
                                        }
                                        if (menu_choice == 4) {
                                                log2file(" custom4 selected");
                                                status = GS_PLAY;
                                                play_sound(sfx[SMPL_MENU]);
                                        }
                                        count = 10;
        ...
        if (status == GS_QUIT_MENU) {
                ...
                textout_outline_center(swap_screen, "MUSIC, SFX:", 80, 46);
                textout_outline_center(swap_screen, "Anders Svensson", 80, 58);
                textout_outline_center(swap_screen, "PSP Port:", 80, 80);
                textout_outline_center(swap_screen, "MK2k", 80, 92);
                blit_to_screen(swap_screen);
                fade_in_pal(100);
                cycle_count = 0;
...
int main(int argc, char **argv) {
        ...
        // Test whether user wants to use a log file
