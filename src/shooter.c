...
void s_draw_object(BITMAP *bmp, Tspace_object *o) {
        if (o->type != SO_EXPLOSION) {
                if (o->type == SO_PLAYER_BULLET) // Argh! - blend bullets (minor eye-candy tweak :)
                ...
                int c = (o->energy + 8) >> 3;
                // Argh! - colors depend on color mode
                if (c > 1) circle(bmp, (int)o->x + 15, (int)o->y + 15, o->image + 9, get_config_int("graphics", "color", 1) ? c + 24 : c);
                if (c > 2) circle(bmp, (int)o->x + 15, (int)o->y + 15, o->image + 8, get_config_int("graphics", "color", 1) ? c + 23 : c - 1);
                if (c > 3) circle(bmp, (int)o->x + 15, (int)o->y + 15, o->image + 7, get_config_int("graphics", "color", 1) ? c + 22 : c - 2);
...
// Argh! - main blend calculation for full color mode
void return_additive_color(AL_CONST PALETTE pal, int x, int y, RGB *rgb)
{
        // add colors (caps within valid range)
        rgb->r = MIN(pal[x].r + pal[y].r, 63);
        rgb->g = MIN(pal[x].g + pal[y].g, 63);
        rgb->b = MIN(pal[x].b + pal[y].b, 63);
}

// Argh! - creates explosion blendmap
void create_blend_map(void) {
        log2file(" creating blend map");

        if (get_config_int("graphics", "color", 1)) { // full color mode blend
                create_color_table(&s_blend_map, org_pal, return_additive_color, NULL);
        }
        else { // 4-color blend (rewrote, yields same results)
                int x, y;
                for (x = 0; x < 5; x++)
                        for (y = 0; y < 5; y++) {
                                if (x == 0)
...
int s_init_shooter() {
        ...
        printf(" reserving buffer memory");
...
void s_uninit_shooter() {
        ...
        printf(" freeing explosion frames");
        for(i = 0; i < 32; i ++)
